...
> 现状说明：
> 本文件主要是历史设计/演进草案，不再完整代表当前仓库实现。
> 当前真实运行架构请优先看 `README.md`、`AGENT.md` 和相关代码。
> 已落地且和本文部分旧描述不同的关键点包括：
> `RabbitMQ` 单轨、物理库存分片、`seckill_reservations/event_outbox/payments` 账本、默认双实例启动、直连 `gRPC` benchmark 作为主性能口径。
