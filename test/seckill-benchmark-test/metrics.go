...
func (m *Metrics) Report(totalDuration time.Duration, totalStock, actualSold int64) {
	...
	reservationRatio := float64(0)
	if totalStock > 0 {
		reservationRatio = float64(actualSold) / float64(totalStock) * 100
	}
	...
	fmt.Println()
	fmt.Println("========================================")
	fmt.Println("   秒杀同步入口性能报告 (Direct gRPC)")
	fmt.Println("========================================")
	fmt.Printf("  调度统计:\n")
	fmt.Printf("    计划请求数:   %d\n", scheduled)
	...
	fmt.Printf("    TPS (完成请求): %.2f req/s\n", tpsCompleted)
	fmt.Printf("    TPS (业务成功): %.2f req/s\n", tpsBusinessSuccess)
	if seckillPhaseTPS > 0 {
		fmt.Printf("    TPS (秒杀阶段): %.2f req/s (%.2fs)\n", seckillPhaseTPS, seckillPhaseDuration)
	}
