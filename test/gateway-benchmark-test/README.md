...
```bash
go run . \
  --gateway-targets=http://127.0.0.1:8888,http://127.0.0.1:18888 \
  --product-id=9101 \
  --stock=15000 \
  --users=100000 \
  --rate=20000 \
...
```bash
go run . --ideal --gateway-targets=http://127.0.0.1:8888,http://127.0.0.1:18888 --rate=20000 --duration=30s
```

`--ideal` 会自动把 `stock/users` 至少提升到 `rate * duration + 1000`，避免过早 SOLD_OUT 或用户重复导致 TPS 被稀释。

仅测网关转发吞吐（推荐用于判断 gateway 是否瓶颈）：
